...
left join harga_edit as c on a.id_he = c.id_he 
	where a.status in ('Dalam Proses','Selesai') and month(a.waktu_pesan)='$bulan' and year(a.waktu_pesan)='$tahun'");
$q_book = mysqli_query($conn, "SELECT * from booking as a
left join paket as b on a.id_paket=b.id_paket
	where a.status in ('Dalam Proses','Selesai') and month(a.tanggal_booking)='$bulan' and year(a.tanggal_booking)='$tahun'");
}else{
	$caption_bulan = '';
...
left join harga_cetak as b on a.id_hc=b.id_hc
left join harga_edit as c on a.id_he = c.id_he 
	where a.status in ('Dalam Proses','Selesai') ");
$q_book = mysqli_query($conn, "SELECT * from booking as a
left join paket as b on a.id_paket=b.id_paket
	where a.status in ('Dalam Proses','Selesai') ");
}
